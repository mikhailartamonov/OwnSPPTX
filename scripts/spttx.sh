...
auto_punctuation=$(awk '$1=="autoPunctuation"{print $2}' "$DATA_FILE")
# Set text normalization based on autoPunctuation
if [ "$auto_punctuation" = "true" ]; then
...
process_file() {
    ...
    [[ -n "$manual_lang" && "$manual_lang" != "ru-RU" ]] && model="general"

    local file_clean
    file_clean=$(sanitize_filename "$file")
    local newfile
    newfile="${file_clean%.*}[[${manual_lang:-ru-RU}]].${file##*.}"
    ...
    cat > params.json <<EOF_JSON
{
  "config": {
    "specification": {
      ...
      ,"textNormalization": { "enabled": "$text_normalization" }
    }
  },
